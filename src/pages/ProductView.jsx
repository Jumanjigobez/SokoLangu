...
const ProductView = () => {
    ...
    const handleCategoryChange = (category) => {
        if (category === "All") {
            ...
            const filteredData = farmerProductsData.filter((product) => product.Category === category);
        ...
        axios({
            ...
            .then((response) => {
                if (response.data == 1) {
                    ...
                    let formData = new FormData();
                    formData.append("ProductId", productID);
                    formData.append("msg", "Hello, I've just placed an order. Kindly review and respond as soon as possible. Thank you!");
    ...
    const handleCalculateTotal = (e) => {
        ...
        if (quantity_input.current.value !== 0) {
            setTotal_Price({
                total: (Number(quantity_input.current.value) * Number(productsData[0].Price))
            })
    ...
    const handleSendMessage = (e, productID) => {
        ...
        formData.append("ProductId", productID);
        formData.append("msg", msg_input.current.value.replace(/(\r\n|\n|\r)/gm, "").trim());
                            ...
                            <>
                                {productsData.length !== 0 ? (
                                    productsData.map((row) => {
                                        const isInWishlist = wishlist.some((item) => item.ProductID === row.Id);
                                        return (
                                            <div className='content' key={row.Id}>
                                                <div className='part_1'>
                                                    <div className='img_part'>
                                                        <NavLink to={`${api_url}/photos/products/${row.Img}`} target='_blank'>
                                                    ...
                                                    <div className='box box2 produceView'>
                                                        <div className='part_1'>
                                                            <div className='txt'>
                                                                <h1>{row.Name}</h1>
                                                                <button className={`btn btn2 ${isInWishlist ? 'active' : ''}`} onClick={(e) => toggleWish(e.currentTarget, row.Id)}>
                                                                    <i className='fa-solid fa-heart'></i>
                                                                </button>
                                                            </div>

                                                            <div className='txt'>
                                                                <p><i className='fa-solid fa-location'></i> {" "}{row.Region}</p>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className='part_2 txt_part'>
                                                            ...
                                                            <div className='btn_part'>
                                                                <button className='btn' onClick={() => openDialogOrder("order", row.Id)}>Place Order</button>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                    ...
                                                    <div className='box box2 produceView'>
                                                        <div className='profile'>
                                                            <div className='name_part'>
                                                                <div className='img_part'>
                                                                    <img src={`${api_url}/photos/profiles/${row.FarmerPhoto}`} alt={row.FarmerFirstName} />
                                                                </div>
                                                                <h2>{row.FarmerFirstName} {" "} {row.FarmerLastName}</h2>
                                                            </div>

                                                            <div className='btn_part'>
                                                                <NavLink to={`tel:${row.FarmerPhone}`}><button className='btn'><i className='fa-solid fa-phone'></i> Make a Call</button></NavLink>
                                                            </div>
                                                        </div>

                                                        <div className='form_part'>
                                                            <form method='post' onSubmit={(e) => handleSendMessage(e, row.Id)}>
                                                                <div className='input_group'>
                                                                    <textarea name="msg" id="msg" cols="30" rows="10" placeholder="Type a message..." required ref={msg_input}></textarea>
                                                                </div>
                                        ...
                                        <>
                                            {
                                                filteredProductsData.length !== 0 ? (
                                                    filteredProductsData.map((row) => (
                                                        <NavLink to={`/productView/${row.Id}`} key={row.Id} onClick={() => window.location.href = `/productView/${row.Id}`}>
                                                            <div className="box">
                                                                <div className="img_part">
                                                                    <img src={`${api_url}/photos/products/${row.Img}`} alt={row.Name} />
                                                                </div>
                                ...
                                >
                                    ...
                                    <div className='input_group'>
                                        <h2 style={{
                                            textAlign: "center",
                                            fontSize: "4em"
                                        }}>Ksh {" "}{Number(Total_Price.total).toLocaleString()}</h2>
                                    </div>
