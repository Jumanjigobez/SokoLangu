...
const MessagesFarmer = () => {
    ...
    const handleUserClick = (receiver) => {
        ...
        axios({
            method: "get",
            url: api_url + "/farmer/updateMessages.php",
            params: {
                user_id: user, // farmer's user ID
                receiver_id: receiver.partner_id // Consumer's user ID as the receiver_id
            }
        }).then((response) => {
    ...
    const handleSendMessage = (e) => {
        ...
        axios.post(api_url + "/farmer/sendChat.php", {
    ...
    const processedMessagesData = (Array.isArray(MessagesData) ? MessagesData : []).reduce((acc, current) => {
        ...
        if (isSender || isReceiver) {
            ...
            const partner_firstName = isSender ? current.receiver_firstName : current.sender_firstName;
            const partner_lastName = isSender ? current.receiver_lastName : current.sender_lastName;
            const partner_photo = isSender ? current.receiver_photo : current.sender_photo;
            ...
            if (!existing) {
                ...
                // Reset unseenCount if the message has been marked as "seen"
                if (isReceiver && current.seen === "Yes" && existing.unseenCount > 0) {
                    existing.unseenCount = 0;
                }
            }
        }
        ...
        <>
            ...
            <section id="active">
                ...
                <div className="container dashContainer">
                    ...
                    <div className="content_part settings_content messages_container">
                                        ...
                                        <div className='users'>
                                            {
                                                processedMessagesData.length !== 0 ? (
                                                    processedMessagesData.map((row) => (
                                                        <div className={`user_box ${selectedUser && selectedUser.partner_id === row.partner_id ? 'active' : ''}`} key={row.partner_id} onClick={() => handleUserClick(row)}>
                                                            <div className='img_part'>
                                                                <img src={api_url + "/photos/profiles/" + row.partner_photo} alt={row.partner_firstName} />
                                                            </div>

                                                            <div className='info_part'>
                                                                <h2>{row.partner_firstName} {" "} {row.partner_lastName}</h2>
                                                                <p style={{ position: "relative" }}>
                                                                    <span
                                                                        dangerouslySetInnerHTML={{
                                                                            __html: user !== row.partner_id
                                                                                ? `<b>You:</b> ${truncateMessage(row.last_message, 30)}`
                                                                                : truncateMessage(row.last_message, 30)
                                                                        }}
                                                                    ></span>
                                                                    {row.unseenCount > 0 && MessagesData.filter((msg) => user === msg.receiver_id && msg.seen === "No").length > 0 && (
                                                                        <span className="msgs_count" style={{ right: "-10%", top: "-10%" }}>{row.unseenCount}</span>
                                                                    )}
                                                                </p>
                                                    ...
                                                    <div className='empty'>
                                                        <p style={{ color: "var(--black)", fontWeight: "bold" }}>No Orders Found!</p>
                                                    </div>
                                                )
                                            }
                                        </div>
                        ...
                        <div className='box box2 msg_container'>
                            <div className='header_part'>
                                {selectedUser ? (
                                    processedMessagesData.length !== 0 && (
                                        <>
                                            <div className='img_part'>
                                                <img src={api_url + "/photos/profiles/" + selectedUser.partner_photo} alt={`${selectedUser.partner_firstName} ${selectedUser.partner_lastName}`} className='user_photo' />
                                            </div>

                                            <div className='name_part'>
                                                <h2>{selectedUser.partner_firstName} {selectedUser.partner_lastName}</h2>
                                            </div>

                                            <div className=''>
                                                <button className='btn btn4 normal' onClick={() => setSelectedUser(null)}><i className='fa-solid fa-times'></i></button>
                                            </div>
                                        </>
                                ...
                                {selectedUser ? (
                                    <div className='msg_content'>
                                        {MessagesData.filter(msg =>
                                            (msg.sender_id === user && msg.receiver_id === selectedUser.partner_id) ||
                                            (msg.sender_id === selectedUser.partner_id && msg.receiver_id === user)
                                        )
                                            .sort((a, b) => new Date(a.date) - new Date(b.date)) // Oldest to newest
                                            .map(msg => (
                                                <div className={`msg_box ${msg.sender_id === user ? 'receiver' : 'sender'}`} key={msg.msg_id}>
                                                    <p>{msg.message}<span className='msg_date'>{msg.date}</span></p>
                                                    {msg.sender_id === user && (
                                                        <span className='msg_delete' onClick={() => handleDeleteMessage(msg.msg_id)}>
                                    ...
                                    <div className='awaiting_box'>
                                        ...
                                        <h2 style={{ color: "var(--black)", fontWeight: "bold" }}>Select a conversation to view messages.</h2>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className='send_part'>
                                {selectedUser && (
                                    <form className='msg_form' onSubmit={(e) => handleSendMessage(e)}>
                                        ...
                                        <div className='btn_part'>
                                            <button type="submit" className='btn btn2'><i className='fa-solid fa-paper-plane'></i></button>
                                        </div>
                                    </form>
                                )}
                            </div>
