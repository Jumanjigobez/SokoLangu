...
const TopHeader = () => {
    ...
    const user = logged ? JSON.parse(localStorage.getItem("sessions")).user_id : "",
        user_role = logged ? JSON.parse(localStorage.getItem("sessions")).user_role : "";
            ...
            <div className='header_content'>
                <div className='content'>
                    <div className='logo_part'>
                        <NavLink to='/'><img src='../images/logo.png' alt='SokoLangu' onClick={() => window.location.href = "/"} /></NavLink>
                    </div>
...

                    {
                        !loggedIn ? (
                            ...
                            <div className="part_2 user_part">
                                ...
                                {/* The Message part*/}
                                <button className="btn btn2 msgs_btn" onClick={() => navigate("/messagesConsumer")}>
                                    <i className="fa-solid fa-comments"></i>
                                    {unseenCount > 0 && messagesData.filter((msg => user === msg.receiver_id)).length > 0 && (
                                        <p className="msgs_count">{unseenCount}</p>
                                    )}
                                </button>
                                ...
                                >
                                    <div className='img_part'>
                                        <img src={accountData.length !== 0
                                            ? api_url + "/photos/profiles/" + accountData.map((row) => row.photo)
                                            : api_url + "/photos/profiles/" + "avatar.png"} alt={accountData.map((row) => row.username)} />
                                    </div>
                                </div>
